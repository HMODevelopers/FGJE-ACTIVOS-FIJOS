...
namespace ActivosFijos.Helpers
{
    public class ConceptosHelper
    {
        ...
        public IPagedList<PLU_CAT_Conceptos> GetAll(string sOrden, string Concepto, int iPagina, int iPerPage)
        {



            var vModel = _db.PLU_CAT_Conceptos.Where(r => (Concepto.Length == 0 || r.NombreConcepto.Contains(Concepto)));

            switch (sOrden)
            {
                case "#":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenBy(r => r.IdConcepto);
                    break;
                case "#_desc":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenByDescending(r => r.IdConcepto);
                    break;
                case "NombreConcepto":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenBy(r => r.NombreConcepto);
                    break;
                case "NombreConcepto_desc":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenByDescending(r => r.NombreConcepto);
                    break;
                default:
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenBy(r => r.IdConcepto);
                    break;
        ...
        public ResponseModel Add(PLU_CAT_Conceptos concepto)
        {
            ...
            try
            {
                using (var ctx = new ModelContext())
                {
                    ...
                    ctx.Entry(concepto).State = EntityState.Added;
                    ctx.SaveChanges();
                    rm.SetResponse(true);
                }
            }
        ...
        public ResponseModel Edit(PLU_CAT_Conceptos concepto)
        {
            var rm = new ResponseModel();
            ...
            try
            {
                using (var ctx = new ModelContext())
                {
                    ...
                    ctx.Entry(concepto).State = EntityState.Modified;
                    ctx.SaveChanges();
                    rm.SetResponse(true);
                }
            }
