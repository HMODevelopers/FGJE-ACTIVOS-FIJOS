...
namespace ActivosFijos.Helpers
{
    public class CategoriasHelper
    {
        ...
        public IPagedList<PLU_CAT_CategoriaActivo> GetAll(string sOrden, string Categoria, int iPagina, int iPerPage)
        {



            var vModel = _db.PLU_CAT_CategoriaActivo.Where(r => (Categoria.Length == 0 || r.NombreCategoria.Contains(Categoria)));

            switch (sOrden)
            {
                case "#":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenBy(r => r.IdCategoria);
                    break;
                case "#_desc":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenByDescending(r => r.IdCategoria);
                    break;
                case "NombreCategoria":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenBy(r => r.NombreCategoria);
                    break;
                case "NombreCategoria_desc":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenByDescending(r => r.NombreCategoria);
                    break;
                default:
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenBy(r => r.IdCategoria);
                    break;
        ...
        public ResponseModel Add(PLU_CAT_CategoriaActivo categoria)
        {
        ...
        public ResponseModel Edit(PLU_CAT_CategoriaActivo categoria)
        {
            var rm = new ResponseModel();
