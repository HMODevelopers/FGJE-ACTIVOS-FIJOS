...
{
    public class MarcasHelper
    {
        ModelContext _db = new ModelContext();

        public IPagedList<PLU_CAT_MarcaActivo> GetAll(string sOrden, string NombreMarca, int iPagina, int iPerPage)
        {


            var vModel = _db.PLU_CAT_MarcaActivo.Where(r => (NombreMarca.Length == 0 || r.NombreMarca.Contains(NombreMarca)));

            switch (sOrden)
            {
                case "#":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenBy(r => r.IdMarca);
                    break;
                case "#_desc":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenByDescending(r => r.IdMarca);
                    break;
                case "NombreMarca":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenBy(r => r.NombreMarca);
                    break;
                case "NombreMarca_desc":
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenByDescending(r => r.NombreMarca);
                    break;
                default:
                    vModel = vModel.OrderByDescending(r => r.Activo).ThenBy(r => r.IdMarca);
                    break;
        ...
        public ResponseModel Add(PLU_CAT_MarcaActivo marca)
        {
        ...
        public ResponseModel Edit(PLU_CAT_MarcaActivo marca)
        {
            var rm = new ResponseModel();
